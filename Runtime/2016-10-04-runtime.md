...
- Bump consul to 0.7
- Consul is no longer orchestrated using `bootstrap-expect` and we are now manually bootstrapping nodes based on leader and member information. This will allow us to automatically recover from quorum loss and split-brain situations.
- Added basic Windows support for consul agents
...
- bbl has been moved to the cloudfoundry github organization
